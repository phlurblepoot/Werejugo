...
- [ ] **Step 2: Write a smoke test for stats + export**

Create `backend/src/routes/aux.test.ts`:
```ts
import { afterAll, beforeAll, expect, test } from "vitest";
import { buildTestApp, closeTestApp, type TestCtx } from "../test/helpers.js";
import { query } from "../db/pool.js";
...
beforeAll(async () => {
  ctx = await buildTestApp();
  await query("INSERT INTO visits (family_id, kind, title) VALUES ($1,'place','A')", [ctx.familyId]);
});
afterAll(async () => { await closeTestApp(ctx); });
const auth = () => ({ authorization: `Bearer ${ctx.token}` });
...
```
> Adjust the URL/asserts to the actual `stats.ts` route path after reading it.
